...
widget!(
    ...
    /// property. An optional distance attribute (float), defines the
...
impl Template for Popup {
    fn template(self, _id: Entity, _: &mut BuildContext) -> Self {
        self.name("Popup")
            ...
            .open(false)
            //.padding(0.0)
            //.on_mouse_down(|_states, _mouse| true)
