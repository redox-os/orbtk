...
use orbtk::{Action, Button, Grid, Image, Label, Menu, Point, ProgressBar, Rect, Separator,
            TextBox, Window};
...
fn main() {
    ...
    menu.position(x, y).size(32, 16);

    y += menu.rect.get().height as i32 + 10;

    let label = Label::new();
    label.position(x, y).size(400, 16).text("Test Label");
    window.add(&label);
    ...
    text_box
        ...
        .size(332, 28)
        .text_offset(6, 6)
        .on_enter(move |text_box: &TextBox| { label.text.set(text_box.text.get()); });
    window.add(&text_box);

    let button = Button::new();
    button
        .position(x + text_box.rect.get().width as i32 + 8, y)
        .size(48 + 12, text_box.rect.get().height)
        .text("Update")
        .text_offset(6, 6)
        .on_click(move |_button: &Button, _point: Point| { text_box.emit_enter(); });
    window.add(&button);
    ...
    offset_label
        ...
        .text("Test Offset")
        .text_offset(50, 50)
        .on_click(|label: &Label, _point: Point| { label.text("Clicked"); });
