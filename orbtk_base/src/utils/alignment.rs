...
impl From<Value> for Alignment {
    fn from(v: Value) -> Self {
        let value = v.into_rust::<String>().expect("Cannot ");
...
mod tests {
    ...
    fn test_into() {
        ...
        let alignment: Alignment = Value::String("Start".to_string()).into();
        assert_eq!(alignment, Alignment::Start);

        let alignment: Alignment = Value::String("start".to_string()).into();
        assert_eq!(alignment, Alignment::Start);

        let alignment: Alignment = Value::String("Center".to_string()).into();
        assert_eq!(alignment, Alignment::Center);

        let alignment: Alignment = Value::String("center".to_string()).into();
        assert_eq!(alignment, Alignment::Center);

        let alignment: Alignment = Value::String("End".to_string()).into();
        assert_eq!(alignment, Alignment::End);

        let alignment: Alignment = Value::String("end".to_string()).into();
        assert_eq!(alignment, Alignment::End);

        let alignment: Alignment = Value::String("Stretch".to_string()).into();
        assert_eq!(alignment, Alignment::Stretch);

        let alignment: Alignment = Value::String("stretch".to_string()).into();
        assert_eq!(alignment, Alignment::Stretch);

        let alignment: Alignment = Value::String("other".to_string()).into();
        assert_eq!(alignment, Alignment::Stretch);
    }
}
